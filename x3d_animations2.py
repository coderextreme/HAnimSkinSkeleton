...
class X3DAutoRigger:
    ...
    def align_scales(self):
        ...
        # 4. Apply Scale to PositionInterpolators to keep translation animations proportionally correct
        if scale_factor != 1.0:
            for node in self.skel_tree.getroot().iter():
                if node.tag.split('}')[-1] == 'PositionInterpolator':
                    if 'keyValue' in node.attrib:
                        vals = parse_vec_array(node.attrib['keyValue'])
                        node.attrib['keyValue'] = format_array(vals * scale_factor)
    ...
    def merge_and_export(self, output_path):
        ...
        # 6. Save Final XML
        # Use the original skeleton's root to preserve TimeSensors, Groups, Interpolators, 
        # and ROUTEs (i.e. keep all animations).
        new_tree_root = self.skel_tree.getroot()
        new_tree_root.attrib['profile'] = "Immersive"
        new_tree_root.attrib['version'] = "4.0"
    ...
    def _cleanup_x3d_tree(self, root_node):
        ...
        removes Transforms under HAnimSegment/HAnimSite, 
        and removes any orphaned USE nodes.
        """
        # Pass 0: Remove Transform elements below HAnimSegment and HAnimSite
        def remove_transforms(parent):
            for child in list(parent):
                if child.tag.split('}')[-1] == 'Transform':
                    parent.remove(child)
                else:
        ...
        for el in root_node.iter():
            tag = el.tag.split('}')[-1]
            if tag in ['HAnimSegment', 'HAnimSite']:
                remove_transforms(el)
